...
import { useNavigate } from 'react-router-dom';
import { createUserWithEmailAndPassword, signInWithEmailAndPassword } from 'firebase/auth';
import { loadGisScript, authenticateYouTube } from '../../components/googleAuth/googleAuthUtils';
import { AuthContext } from '../../components/AuthProvider';
import { doc, getDoc, setDoc } from 'firebase/firestore';
import axios from 'axios';
...
export default function LoginPage() {
  ...
  const toggleSignUp = () => setFormTitle('SignUp');
    ...
    <div className="login">
      <div className='login_container'>
        ...
        <div className='login_form'>
          <div className='login_title'>
            <h1>{formTitle === 'Login' ? 'Login' : 'SignUp'}</h1>
            <p>{formTitle === 'Login' ? 'Login with your email ' : 'SignUp with your email'}</p>
          </div>
          <div className='form_container'>
            <Form onSubmit={formTitle === 'Login' ? handleLogIn : handleSignUp}>
              <FormGroup controlId='email'>
              ...
              <div className='button_container'>
                <Button className='signup_button' onClick={toggleSignUp}>
                  {formTitle === 'Login' ? 'Create account' : ''}
                </Button>
                <Button type='submit'>
                  {formTitle === 'Login' ? 'Login' : 'Sign up'}
                </Button>
              </div>
            </Form>
          </div>
